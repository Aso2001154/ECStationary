...
<head>
    ...
    <script>
        let btnCheck = () =>{
            ...
            if(beforeGoods.match(/[a-zA-Z0-9あ-ん-ー*@/()$#&!%<>]{1,50}/) && beforeDetail.match(/[a-zA-Z0-9あ-ん-ー*@/()$#&!%<>]{1,500}/) {
                if(beforeGoods.match(/[a-zA-Z0-9あ-ん-ー*@/()$#&!%<>]{51,}/) || beforeDetail.match(/[a-zA-Z0-9あ-ん-ー*@/()$#&!%<>]{501,}/) {
                    swal('文字数をオーバーしています');
                    return false;
                }else{
...
<!--form-->
<form action="merchandise_result.php" method="post" enctype="multipart/form-data">
    <div class="container">
        <h1 class="title">new merchandise</h1>
        <div class="range">
        <p for="genre" class="subtitle">ジャンル</p>
        ...
        <p for="form" class="subtitle">商品名<span class="span">※文字数：50文字以内/すでに登録されている商品名だと登録することはできません。</span></p>
            <p class="merchandise"><input type="text" name="goods" id="goods" class="text"></p>

        <p for="form" class="subtitle">価格<span class="span">※文字種：数字/制限：ハイフンはなし</span></p>
            <p class="merchandise"><input type="number" name="price" id="price" class="number_text"></p>

        <p for="form" class="subtitle">画像</p>
            <p class="merchandise"><input type="file" name="file" class="file_text" ></p>
        <p for="form" class="subtitle">商品詳細<span class="span">※文字数：500文字以内</span></p>
            <p class="merchandise"><textarea name="detail" rows="5" cols="50" id="detail" class="textarea"></textarea></p>
        <button type="submit" value="send" class="btn" onsubmit="return btnCheck()" onclick="return btnCheck()">register</button>
        </div>
</form>
