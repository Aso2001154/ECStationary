...
if($period == 7 || $period == 31){
    ...
    $sql = $pdo->prepare(history_period());//別のファイルから呼び出し
    ...
    if($cnt!=0) {
        ...
        $sql1 = $pdo->prepare(history_period_detail());//別のファイルから呼び出し
    ...
    // 絞込みがなく全て履歴を抽出する
    $sql = $pdo->prepare(history_all());//別のファイルから呼び出し
    ...
    if($cnt!=0) {
        ...
        $sql1 = $pdo->prepare(history_all_detail());//別のファイルから呼び出し
