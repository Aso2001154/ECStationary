...
$form_history = '<p class="btn_p"><button type="submit" class="btn" onclick="btnCheck()">履歴</button></p>';
if(isset($_SESSION['user'])){ // userセッションにデータが入っていればログイン状態
    ...
    $form_history1 = '<p class="btn_p"><button type="submit" class="btn">履歴</button></p>';
}

try {
    // データベースログインの為の情報を関数化し別のファイルから呼んでいる
    require "data_base.php";
    $pdo = data_base();
    $sql = merchandise(); // ジャンルによって出力する商品を変えるsql文が入っている
    ...
    $i=1;
    if($genre_id==1){ $img='シャーペン';} // ジャンルIDによって出力する背景をここで判断して変えている
...
<?php // ログインしていない時としている時でとんでいくページが違う
